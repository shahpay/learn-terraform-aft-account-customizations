...
resource "aws_securityhub_standards_control" "disabled_control_all" {
   ...
   standards_control_arn = "arn:aws:securityhub:${data.aws_region.current_region.name}:${data.aws_caller_identity.current_account.account_id}:control/standards/nist-800-53/v/${locals.nis_version}/${each.value.control_id}"
}